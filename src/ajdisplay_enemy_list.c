...
void ajd_enemy_list(enemy_t *_enemy, sfRenderWindow *_window)
{
    if (_enemy->next == NULL)
        ...
        while (_enemy->next != NULL) {
            ...
            sfRectangleShape_setSize(dbg_hb, (sfVector2f){_enemy->hitbox.width, _enemy->hitbox.height});
            sfRectangleShape_setFillColor(dbg_hb, sfGreen);
            sfRectangleShape_setPosition(dbg_hb, (sfVector2f){_enemy->hitbox.left, _enemy->hitbox.top});
