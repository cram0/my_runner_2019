...
{
    switch (enemy_type) {
        case BAT: return ((sfVector2f){0, 0});
            break;
        case WEREWOLF: return ((sfVector2f){0, 0});
            break;
        case PANTHER: return ((sfVector2f){0, 0});
            break;
    }
}
...
void add_node_enemy(enemy_t *_enemies, int enemy_id, enemy_types *_types)
{
...
void aji_enemy_list(enemy_t *_enemies, char *map, enemy_types *_enemy_type)
{
    ...
    while (map[map_i++]) {
        ...
        {
        case '1':
            add_node_enemy(_enemies, BAT, _enemy_type);
            break;
        case '2':
            add_node_enemy(_enemies, WEREWOLF, _enemy_type);
            break;
        case '3':
            add_node_enemy(_enemies, PANTHER, _enemy_type);
        default:
            break;
        }
    }
